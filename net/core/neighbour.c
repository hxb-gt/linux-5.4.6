			  ...
			  struct netlink_ext_ack *extack)
{
	...
	if (!(flags & NEIGH_UPDATE_F_ADMIN) && /*  neigh NOARP 和PERMANENT 标志只有ADMIN 用户能够修改  */
	...
	ext_learn_change = neigh_update_ext_learned(neigh, flags, &notify);/* 如果neigh是学习标志更新  */

	if (!(new & NUD_VALID)) { /*  如果更新标志不是有效，则删除neigh  */
		neigh_del_timer(neigh);
		...
		if ((old & (NUD_INCOMPLETE | NUD_PROBE)) && /*  如果新标志是FAILED，则清楚neigh 上面的请求  */
		    (new & NUD_FAILED)) {
			neigh_invalidate(neigh);
			notify = 1;
		}
	...
	if (old & NUD_VALID) {/*  原来的状态valid  */
		if (lladdr != neigh->ha && !(flags & NEIGH_UPDATE_F_OVERRIDE)) {/*  如果更新地址，但是没有设置override标志  */
			update_isrouter = 0;
			if ((flags & NEIGH_UPDATE_F_WEAK_OVERRIDE) &&
			    (old & NUD_CONNECTED)) {/*  地址不同，触发可达性验证  */
				lladdr = neigh->ha;
				new = NUD_STALE;
			} else
				goto out;
	...
	if (lladdr != neigh->ha) {/*  地址更新  */
		write_seqlock(&neigh->ha_lock);
		memcpy(&neigh->ha, lladdr, dev->addr_len);
		write_sequnlock(&neigh->ha_lock);
		neigh_update_hhs(neigh);
		if (!(new & NUD_CONNECTED))/*  移除confirm标识  */
			neigh->confirmed = jiffies -
...
out:
	if (update_isrouter)/*  是route 更新 neigh  */
		neigh_update_is_router(neigh, flags, &notify);
	write_unlock_bh(&neigh->lock);

	if (((new ^ old) & NUD_PERMANENT) || ext_learn_change) /*  如果更新了PERMANENT和LEARNED 标志，gc链表会定期被刷掉   */
