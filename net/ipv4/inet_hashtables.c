...
bool inet_ehash_insert(struct sock *sk, struct sock *osk)
{
	...
	head = inet_ehash_bucket(hashinfo, sk->sk_hash);/*  获取ehash头  */
	list = &head->chain;
...
bool inet_ehash_nolisten(struct sock *sk, struct sock *osk)/*  把对应的sk插入ehash  */
{
...
other_parity_scan:
	...
	for (i = 0; i < remaining; i += 2, port += 2) {
		 ...
		 */
		inet_bind_bucket_for_each(tb, &head->chain) {/*  port是否有在bindhash表中  */
		...
		tb = inet_bind_bucket_create(hinfo->bind_bucket_cachep,/*  添加对应的bind对象  */
		      ...
		      struct sock *sk)
{
	...
	if (!inet_sk(sk)->inet_num)/*  获取随机port  */
