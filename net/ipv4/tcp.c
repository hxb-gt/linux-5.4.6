				...
				struct ubuf_info *uarg)
{
	...
	if (inet->defer_connect) {/*  延迟连接inet，现在发送数据，所以开始连接  */
		err = tcp_connect(sk);
	...
	err = __inet_stream_connect(sk->sk_socket, uaddr,/*  sock状态为connecting，此函数返回EINPROGRESS错误  */
...
new_segment:
		...
		if (skb_availroom(skb) > 0 && !zc) {
			...
			if (!skb_can_coalesce(skb, i, pfrag->page,
					      pfrag->offset)) {
				if (i >= sysctl_max_skb_frags) {5232625 2
					tcp_mark_push(tp, skb);
					goto new_segment;
				}
				merge = false;
