...
static int ip_frag_queue(struct ipq *qp, struct sk_buff *skb)
{
	...
	err = pskb_trim_rcsum(skb, end - offset);/*  把skb 去掉ip头和其他数据，变为ip数据报有效载荷  */
	if (err)
		goto discard_qp;
	...
	err = inet_frag_queue_insert(&qp->q, skb, offset, end);/*把当前skb插入片段表中*/
	if (err)
		goto insert_error;
	...
	if (qp->q.flags == (INET_FRAG_FIRST_IN | INET_FRAG_LAST_IN) && /*  组装分片  */
	    qp->q.meat == qp->q.len) {
