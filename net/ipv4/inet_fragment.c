			   ...
			   int offset, int end)
{
	...
	if (!last)/* 第一个片段*/
		...
		if (offset == last->ip_defrag_offset + last->len)/*  如果是连续的offset，则放入一个链表中  */
			fragrun_append_to_last(q, skb);
		else
			fragrun_create(q, skb);/*  插入红黑树中  */
	} else {
...
void *inet_frag_reasm_prepare(struct inet_frag_queue *q, struct sk_buff *skb,/*  准备head skb  */
			      struct sk_buff *parent)
{
	...
	if (head != skb) {
		fp = skb_clone(skb, GFP_ATOMIC);/*  先克隆skb的信息到fp，因为skb要作为head为后续用  */
		if (!fp)
			return NULL;
		...
		if (q->fragments_tail == skb)
			q->fragments_tail = fp;
		skb_morph(skb, head);/*  把head的信息复制到skb中  */
	 ...
	 */
	if (skb_has_frag_list(head)) {/*  如果head有分片，则拆分成两个skb，一个保存线性数据，一个保存分片  */
			    ...
			    void *reasm_data, bool try_coalesce)
{
	...
	skb_push(head, head->data - skb_network_header(head));/*  指向head的L3头  */

	/* Traverse the tree in order, to build frag_list. */
	fp = FRAG_CB(head)->next_frag; /*  片段位于两个位置，一个是fp，也就是head，另外一个在红黑树中  */
