					      ...
					      struct request_sock *req)
{
	...
	child = inet_csk(sk)->icsk_af_ops->syn_recv_sock(sk, skb, req, NULL,/*  创建一个全功能的sock  */
							 NULL, &own_req);
	if (!child)
		return NULL;
	...
	tcp_fastopen_add_skb(child, skb);/*  fastopen 接受数据部分  */
			      ...
			      const struct dst_entry *dst)
{
	...
	if (!((tcp_fastopen & TFO_SERVER_ENABLE) &&/*  判断是否是fastopen  */
	      (syn_data || foc->len >= 0) &&
	...
	if (foc->len == 0) {
		...
		tcp_fastopen_cookie_gen(sk, req, skb, &valid_foc);/*  生成秘钥  */
	} else if (foc->len > 0) {
		ret = tcp_fastopen_cookie_gen_check(sk, req, skb, foc,/*  验证cookes  正确性  */
						    &valid_foc);
		if (!ret) {
...
bool tcp_fastopen_defer_connect(struct sock *sk, int *err)
{
	...
	if (tp->fastopen_connect && !tp->fastopen_req) {
		if (tcp_fastopen_cookie_check(sk, &mss, &cookie)) {
			inet_sk(sk)->defer_connect = 1;/*  延迟发送syn直到有数据写入  */
			return true;
		}
