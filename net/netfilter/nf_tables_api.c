			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (nla[NFTA_CHAIN_HANDLE]) {/*  查找chain  */
	...
	if (nla[NFTA_CHAIN_POLICY]) {/*  添加policy  */
		if (chain != NULL &&
		    !nft_is_base_chain(chain)) {
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	n = 0;
	size = 0;
	if (nla[NFTA_RULE_EXPRESSIONS]) {/*  解析rule的表达式  */
		...
		nla_for_each_nested(tmp, nla[NFTA_RULE_EXPRESSIONS], rem) {
			...
			if (n == NFT_RULE_MAXEXPRS)
				goto err1;
			err = nf_tables_expr_parse(&ctx, tmp, &info[n]);/*  解析expr  表达式  */
			if (err < 0)
				goto err1;
	...
	if (nla[NFTA_RULE_USERDATA]) {/*  用户数据  */
	...
	if (nlh->nlmsg_flags & NLM_F_REPLACE) {/*  替换一个rule  */
		...
		if (nlh->nlmsg_flags & NLM_F_APPEND) {/*  扩充  */
			if (old_rule)
...
static int nf_tables_commit(struct net *net, struct sk_buff *skb)
{
	...
	list_for_each_entry_safe(trans, next, &net->nft.commit_list, list) {/*  准备next generation active状态的rule，为commit做准备  */
		int ret;
