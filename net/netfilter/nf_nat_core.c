...
static int
nf_nat_used_tuple(const struct nf_conntrack_tuple *tuple,/*  是否已经有ct在使用tuple的信息了  */
...
static int
find_appropriate_src(struct net *net,/*  获取一个近似的tuple  */
		     ...
		     const struct nf_nat_range2 *range)
{
	...
	hlist_for_each_entry_rcu(ct, &nf_nat_bysource[h], nat_bysource) {
		if (same_src(ct, tuple) &&
			...
			result->dst = tuple->dst;/*  使用hash值相同tuple的src信息  */
		 ...
		 enum nf_nat_manip_type maniptype)
{
	 ...
	 * manips not an issue.
	 */
	if (maniptype == NF_NAT_MANIP_SRC &&/* 源nat  */
	    !(range->flags & NF_NAT_RANGE_PROTO_RANDOM_ALL)) {
		/* try the original tuple first */
		if (in_range(orig_tuple, range)) {/*  ct连接在提供的range范围内，相当于没有进行nat转换  */
			if (!nf_nat_used_tuple(orig_tuple, ct)) {/*  根据提供的信息是否能在hash表中找到重复的对象  */
				*tuple = *orig_tuple;/*  没有找到信息相同的ct，使用当前tuple  */
				return;
			}
		} else if (find_appropriate_src(net, zone,/*  在已有的源nat 表中查找合适的tuple  ，使用其src信息*/
	...
	*tuple = *orig_tuple;
	find_best_ips_proto(zone, tuple, range, ct, maniptype);/*  根据range选择合适的ip地址  */
	...
	if (!(range->flags & NF_NAT_RANGE_PROTO_RANDOM_ALL)) {/*  如果不是使用随机端口的情况下  */
		if (range->flags & NF_NAT_RANGE_PROTO_SPECIFIED) {
			if (!(range->flags & NF_NAT_RANGE_PROTO_OFFSET) &&
			    ...
			    (range->min_proto.all == range->max_proto.all ||
			     !nf_nat_used_tuple(tuple, ct)))/*  判断此时是否已经得到了一个唯一的tuple  */
				return;
		  ...
		  enum nf_nat_manip_type maniptype)
{
	...
	nf_ct_invert_tuple(&curr_tuple,
			   &ct->tuplehash[IP_CT_DIR_REPLY].tuple);/*  得到原始tuple  */

	get_unique_tuple(&new_tuple, &curr_tuple, range, ct, maniptype);/*  根据range的信息得到唯一的tuple，range是用户写入的nat信息  */
			   ...
			   struct sk_buff *skb)
{
	...
	if (dir == IP_CT_DIR_REPLY)/*  如果是回复包，这nat方向倒转  */
		statusbit ^= IPS_NAT_MASK;
