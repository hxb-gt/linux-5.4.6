...
begin:
	...
	hlist_nulls_for_each_entry_rcu(h, n, &ct_hash[bucket], hnnode) {
		...
		if (nf_ct_key_equal(h, tuple, zone, net))/*  如果相等则说明找到对应的tuple  */
			return h;
	}
	/*
			 ...
			 const struct nf_conn *ignored_conntrack)
{
	...
	rcu_read_lock();
 begin:
	nf_conntrack_get_ht(&ct_hash, &hsize);/*  获取整个hash表  */
	hash = __hash_conntrack(net, tuple, hsize);/*  获取跟踪链表头  */
	       ...
	       unsigned int dataoff, u32 hash)
{
	...
	if (!nf_ct_invert_tuple(&repl_tuple, tuple)) {/*  转换成对端的tuple对象  */
		  ...
		  const struct nf_hook_state *state)
{
	...
	if (!nf_ct_get_tuple(skb, skb_network_offset(skb),/*  从skb提取nf_conntrack_tuple 信息  */
	...
	h = __nf_conntrack_find_get(state->net, zone, &tuple, hash);/*  找到对应的contract  */
	if (!h) {
	...
	if (NF_CT_DIRECTION(h) == IP_CT_DIR_REPLY) {/*  如果根据skb的地址信息找到的是reply tuple，说明连接稳定状态  */
...
repeat:
	ret = resolve_normal_ct(tmpl, skb, dataoff,/*  sets skb->_nfct | ctinfo   */
				protonum, state);
	if (ret < 0) {
